...
| **10** | Tune + Select (val) | `features.parquet` + split + `config/feature_sets.json` → `config/hparams.json` | tune hyper từng model, cho **mỗi `feature_set` ∈ {l1, eda, full}**; chấm val. **Tiêu chí chọn KHOÁ = MCC (chính) + balanced-acc; logloss tie-break; TUYỆT ĐỐI không dùng accuracy thô** (val là regime tăng mạnh pct_pos k=20≈83% còn test≈43% → accuracy thô đánh lừa). Deploy config = (feature_set × model) thắng — best single |
